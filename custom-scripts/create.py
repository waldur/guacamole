...
def insert_to_guacamole_db(instance_uuid, name, email, ip, cursor):
	...
	if cursor.fetchone() is not None:
		print("ERROR: Already can find this connection in gaucamole")
		return None

	cursor.execute("INSERT INTO guacamole_connection (connection_name, protocol, backend_id) \
		VALUES ('{}', 'rdp', '{}')".format(name, instance_uuid))

	connection_id = cursor.lastrowid

	cursor.execute("insert into guacamole_connection_parameter (connection_id, parameter_name, parameter_value) values \
		({}, 'hostname', '{}'), \
		({}, 'ignore-cert', 'true'), \
		({}, 'username', 'user'), \
		({}, 'port', 3389), \
		({}, 'password', '{}')".format(connection_id, ip, connection_id, connection_id, connection_id, connection_id, os.environ.get('DESKTOP_PASSWORD')))

	# Ignore error if email already exists
	cursor.execute("INSERT IGNORE INTO guacamole_entity (name, type) VALUES ('{}', 'USER')".format(email))

	cursor.execute("insert into guacamole_connection_permission (entity_id, connection_id, permission) values \
		((select entity_id from guacamole_entity where name='{}'), {}, 'READ')".format(email, connection_id))
def main():
